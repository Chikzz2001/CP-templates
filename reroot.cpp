...
//X-->number of states u need for transition from dp[nei] to dp[node]
    using Aggregate = std::array<long long, X>;
    using Value = std::array<long long, X>;
    ...
    auto response = rerooter(g, base, merge_into, finalize_merge);
    
    //the ultimate reroot result for all nodes as root
    auto reroot_result=get<0>(response);
    auto edge_dp=get<1>(response);
    auto redge_dp=get<2>(response);